...
text = open('dialogue.txt').read()[:300000]  # More data
chars = sorted(set(text))
stoi = {c: i for i, c in enumerate(chars)}
data = torch.tensor([stoi[c] for c in text], dtype=torch.long)

BATCH, SEQ_LEN, EPOCHS = 32, 128, 5

model = TinyLLM(len(chars), dim=128, n_layers=3, n_heads=4, max_len=SEQ_LEN)
...
torch.save({'model': model.state_dict(), 'chars': chars, 'dim': 128, 'n_layers': 3, 'n_heads': 4, 'max_len': SEQ_LEN}, 'tiny_llm.pt')
